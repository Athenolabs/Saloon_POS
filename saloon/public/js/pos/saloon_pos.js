...
erpnext.pos.PointOfSale = Class.extend({
		...
		if (no_of_items == 0)
			...
			this.with_modes_of_payment(function() {
				...
				// if (dialog.get_value("mode_of_payment")=='Cash'){
					...
					frappe.call({
						method: 'saloon.custom_py_method.get_currency_domination',
						args: {
	...
	total_received: function(dialog){
		...
		var total_return=0
		$(dialog.wrapper).find("#currency_dialog").find(".received").each(function(){
			total_received=total_received+parseFloat($(this).val())
		})
		$(dialog.wrapper).find("#currency_dialog").find(".return").each(function(){
			total_return=total_return+parseFloat($(this).val())
		})
		paid_amount=total_received - total_return
		var values = dialog.get_values();
